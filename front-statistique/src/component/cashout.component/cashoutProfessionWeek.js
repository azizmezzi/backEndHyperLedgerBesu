...
class CashoutProfessionWeek extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    axios
     .get("http://3.88.223.95:3006/Cashout/getProfessionByWeek/0/Commercial/"+this.props.select)
       .then(commericialCount => {
         this.setState({ commericialCount: commericialCount.data.utilisation });
    axios
     .get("http://3.88.223.95:3006/Cashout/getProfessionByWeek/0/Agriculture/"+this.props.select)
       .then(agricultureCount => {
         this.setState({ agricultureCount: agricultureCount.data.utilisation });
   axios
     .get("http://3.88.223.95:3006/Cashout/getProfessionByWeek/0/Artisanat/"+this.props.select)
       .then(artisanatCount => {
         this.setState({ artisanatCount: artisanatCount.data.utilisation });
   axios
     .get("http://3.88.223.95:3006/Cashout/getProfessionByWeek/0/Salarie/"+this.props.select)
       .then(salarieCount => {
...
export default CashoutProfessionWeek;
