...
class CashoutWebMobileYear extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    axios
      .get("http://3.88.223.95:3006/Cashout/getUsesByYear/0/"+this.props.select)
      .then(res => this.setState({ allCashouts: res.data.utilisation }));
    axios.get("http://3.88.223.95:3006/Cashout/getMobileByYear/0/"+this.props.select).then(res => {
      var newData = {
        labels: ["Web", "Mobile"],
        datasets: [
          {
...
export default CashoutWebMobileYear;
