...
class TontineEtatWeek extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    ...
    for (let i = -8; i < 1; i++) {
        enCours.push(
        await axios
          .get("http://3.88.223.95:3006/tontine/getEnCoursByWeek/"  + -i+"/"+this.props.select)
          .then((res) => {
            return res.data;
          })
      );
      termine.push(
        await axios
          .get("http://3.88.223.95:3006/tontine/getTermineByWeek/" + -i+"/"+this.props.select)
          .then((res1) => {
            return res1.data;
          })
      );
...
export default TontineEtatWeek;
