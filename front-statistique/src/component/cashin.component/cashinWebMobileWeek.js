...
class CashinWebMobileWeek extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  async componentDidMount() {
    axios
      .get("http://3.88.223.95:3006/Cashin/getUsesByWeek/0/"+this.props.select)
      .then(res => this.setState({ allCashins: res.data.utilisation }));
    axios.get("http://3.88.223.95:3006/Cashin/getMobileByWeek/0/"+this.props.select).then(res => {
      var newData = {
        labels: ["Web", "Mobile"],
        datasets: [
          {
...
export default CashinWebMobileWeek;
